#!/usr/bin/env python3
"""
Polymarket BTC 5-Min Up/Down — Live Trader
==========================================
Continuously monitors live BTC Up/Down 5-minute markets on Polymarket,
computes fair value using a GBM model, and trades when the edge exceeds
the threshold.
...
Usage:
    python live_trader.py              # paper trading (default)
    python live_trader.py --live       # real trading via CLOB
...
# Config
# ---------------------------------------------------------------------------
THRESHOLD = 0.50         # relative edge threshold (edge / token cost)
...
# --- v2 signal filters (from trade history analysis) ---
MIN_ELAPSED_MINUTES = 2.0   # don't enter in first N minutes (model=0.50 is useless)
...
PROFIT_TARGET = 0.75        # take profit at 75% return on cost basis
MAX_VOL = 0.0010            # skip high-vol regimes (low vol = +$7.39, high vol = +$1.38)
...
OBS_CSV = OUTPUT_DIR / "live_observations.csv"
TRADE_CSV = OUTPUT_DIR / "live_trades.csv"
LOG_FILE = OUTPUT_DIR / "live_trader.log"
...
class BTCFeed:
    ...
    def refresh(self):
        now = time.time()
        # Always fetch the real-time ticker
    ...
    def momentum(self, n_minutes=MOMENTUM_LOOKBACK):
        if len(self.candles) < n_minutes + 1:
            return 0.0
        closes = [c["close"] for c in list(self.candles)[-(n_minutes + 1):]]
        return float(np.log(closes[-1] / closes[0]) / n_minutes)
...
def main():
    parser = argparse.ArgumentParser(description="Polymarket BTC 5-Min Paper Trader")
    parser.add_argument("--live", action="store_true", help="Enable real trading (Phase 2)")
    ...
    parser.add_argument("--exit-threshold", type=float, default=EXIT_THRESHOLD,
                        help="Close position when remaining edge drops below this")
    ...
    parser.add_argument("--min-hold", type=int, default=MIN_HOLD_SECONDS,
                        help="Minimum seconds to hold before early exit")
    ...
    parser.add_argument("--profit-target", type=float, default=PROFIT_TARGET,
                        help="Take profit at this fraction of cost (0=off)")
    parser.add_argument("--max-vol", type=float, default=MAX_VOL,
                        help="Skip trades when 1-min vol exceeds this (0=off)")
    ...
    log.info("Starting %s trader — threshold=%.2f  size=$%.2f  max_loss=$%.2f",
    ...
    while not _shutdown:
        try:
            ...
            clob_mid = None
            if mkt_data and mkt_data["up_token_id"]:
                clob_mid = fetch_clob_midprice(mkt_data["up_token_id"])
            ...
            model_price = fair_price_up(spot, strike, tau_min, sigma_ewma, drift) if tau_min > 0 else None
            ...
            if signal != "NONE":
                if elapsed_min < min_elapsed:
                    ...
                    signal = "NONE"
                elif max_vol > 0 and sigma_ewma > max_vol:
                    filter_reason = f"high_vol ({sigma_ewma:.6f})"
                    signal = "NONE"
            ...
            if signal != "NONE" and win_start not in traded_windows and mkt_data and not paused:
                ...
                if live_order_ok:
                    ...
                    log.info("%s: %s %s @ %.4f  edge=%.4f  rel=%.2f  kelly=$%.2f  BTC=$%.2f",
                             prefix, side, mkt_data["slug"], entry_price, edge,
                             rel_edge or 0, current_trade_size, spot)

            # --- Position management: profit-taking + edge-reversal exit ---
            if win_start in pending_resolution and market_price is not None and model_price is not None:
                ...
                if side == "BUY_UP":
                    ...
                    remaining_edge = model_price - market_price
                else:
                    cost = 1.0 - entry
                    current_value = 1.0 - market_price
                    remaining_edge = market_price - model_price

                unrealized_pct = (current_value - cost) / cost if cost > 0.001 else 0.0
                exit_price = None
                reason = None

                # (1) Profit-taking: capture gains at target return
                if profit_target > 0 and unrealized_pct >= profit_target:
                    exit_price = current_value
                    reason = "profit_target"

                # (2) Edge-reversal stop: exit if model says we're wrong
                elif remaining_edge < exit_threshold and (now_ts - entry_ts) >= min_hold_seconds:
                    exit_price = current_value
                    reason = "edge_reversed" if remaining_edge < 0 else "edge_lost"
                ...
                if exit_price is not None and reason is not None:
                    ...
                    log.info("EXIT [%s]: %s %s  entry=%.4f  exit=%.4f  ret=%.0f%%  pnl=$%.4f  cum=$%.2f",
                             reason, side, trade["slug"], entry,
                             exit_price, unrealized_pct * 100, pnl_usd, cum_pnl)
