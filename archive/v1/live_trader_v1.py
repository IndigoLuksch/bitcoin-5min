...
def main():
    ...
    while not _shutdown:
        try:
            ...
            model_price = fair_price_up(spot, strike, tau_min, sigma, drift) if tau_min > 0 else None
            edge = (model_price - market_price) if (model_price is not None and market_price is not None) else None

            # --- Determine signal (relative edge) ---
            signal = "NONE"
            rel_edge = None
            ...
            # --- Early exit: close position if edge has reversed ---
            if win_start in pending_resolution and market_price is not None and model_price is not None:
                ...
                # Compute remaining edge in the direction of our position
                if side == "BUY_UP":
                    remaining_edge = model_price - market_price
                else:  # BUY_DOWN
                    remaining_edge = market_price - model_price

                # Only exit if edge has genuinely reversed AND we've held long enough
                if remaining_edge < exit_threshold and (now_ts - entry_ts) >= min_hold_seconds:
                    # Sell back at current market price
                    if side == "BUY_UP":
                        exit_price = market_price   # sell Up token
                        gross = exit_price - entry   # bought at entry, sold at exit
                    else:
                        exit_price = 1.0 - market_price  # Down token price
                        entry_cost = 1.0 - entry         # what we paid for Down
                        gross = exit_price - entry_cost
                    ...
                    log.info("EARLY EXIT: %s %s  entry=%.4f  exit=%.4f  reason=%s  pnl=$%.4f  cum=$%.2f",
                             side, trade["slug"], entry,
                             exit_price if side == "BUY_UP" else (1.0 - market_price),
                             reason, pnl_usd, cum_pnl)
